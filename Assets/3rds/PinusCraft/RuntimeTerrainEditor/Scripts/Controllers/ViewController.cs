...
namespace RuntimeTerrainEditor
{
    public class ViewController : MonoBehaviour
    {
        ...
        public Material m_ProjectionMat;
        /// <summary>
        /// 碰撞信息缓存
        /// </summary>
        ...
        private Projector Projection
        {
            get
            {
                if (projector == null)
                {
                    ...
                    projector.material = m_ProjectionMat;
                }
                return projector;
            }
        }
        ...
        private string GetCnModeName(BrushMode brushMode)
        {
            return ((BrushModeCn)((int)brushMode)).ToString();
        }
        ...
        public void ListenInputs()
        {
            if (Projection.enabled)
            {
                ...
                if (!hitInfo.Equals(default(RaycastHit)))
                {
                    ...
                    Projection.orthographicSize = toolView.sizeSlider.value / 2.0f;
                }
            }
        }
        ...
        {
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out RaycastHit hitInfo, float.MaxValue,1 << layer))
            {
                return hitInfo;
            }
            else
        ...
        private void SetupToolPanel()
        {
            ...
            {
                var od = new Dropdown.OptionData(GetCnModeName(_modes[i]));
        ...
        private void SetBrushSelected(SelectionItem selection)
        {
            ...
            m_ProjectionMat.SetTexture("_MainTex", selection.image.texture);
